...
## TEMPEST traps are 0.5 m2
## so calculate g/m2 for all mass
         Year = year(Date_collected),
         Leaf_ACRU / 0.5, Leaf_LITU / 0.5, Leaf_FAGR / 0.5, 
         Leaf_Other / 0.5, All_Leaf / 0.5) %>% 
...
litter_ann_perplot_m2 <- litter_annual_fluxes %>%
  ...
  filter(Year > 2018) %>% 
  group_by(Plot, Year) %>%
  # TEMPEST litter traps are 0.5 m2
  # so we divide each trap mass by 0.5 m2 to get grams per m2
  #average flux across traps
  summarise(All_Leaf_perplot = mean (All_Leaf_peryear / 0.5, na.rm = TRUE),
            .groups = "drop")
