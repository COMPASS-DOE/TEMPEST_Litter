...
```{r}
## Next steps:
# 1. extend litter control A example to plot total litter biomass by year; take litter adjust and compute total flux for each year
...
days_wanted_adj <- seq(from = min(litter_C_A_adjusted$Date_collected), 
                   to = max(litter_C_A_adjusted$Date_collected), 
                   by = "day")

litter_C_A_adjusted %>%
  complete(Plot, Trap, Date_collected = days_wanted_adj) %>%
  mutate(All_Leaf = na.approx(All_Leaf, na.rm = FALSE)) ->
  litter_C_A_adj_allyrs


# 2. plot

ggplot(litter_C_A_adj_allyrs, aes(x = Date_collected, y = All_Leaf)) +
  geom_point()
...
# sum up the daily fluxes to an annual values
  summarise(All_Leaf_peryear = sum(All_Leaf), .groups = "drop") %>%
# plot
  ggplot(aes(Year, All_Leaf_peryear, color = Plot)) + geom_point() + geom_line()
