...
litter %>%
  ...
  mutate(Date_collected = mdy(Date_collected),
         ...
         Leaf_ACRU = Leaf_ACRU / 0.5, 
         Leaf_LITU = Leaf_LITU / 0.5, 
         Leaf_FAGR = Leaf_FAGR / 0.5, 
         Leaf_Other = Leaf_Other / 0.5, 
         All_Leaf = All_Leaf / 0.5) %>% 
  ## finally, isolate columns we care about
...
litter_ann_perplot_m2 <- litter_annual_fluxes %>%
  ...
  filter(Year > 2018) %>% 
  group_by(Plot, Year) %>%
  summarise(All_Leaf_perplot = mean (All_Leaf_peryear, na.rm = TRUE),
            .groups = "drop")
...
ggplot(litter_precip_year, aes(x = Precip_cm, y = All_Leaf_perplot, color = Plot)) +
  geom_point() +
...
ggplot(litter_par_year, aes(x = Srad_Ws_m2, y = All_Leaf_perplot, color = Plot)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    x = "PAR (?)",
