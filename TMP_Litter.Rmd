...
ggplot(litter_par_year, aes(x = Srad_Ws_m2, y = value_perplot, color = Plot)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    x = "PAR (w/m2)",
    y = "Leaf Litter (g/m2/year)",
    color = "Plot"
  ) +
   scale_color_manual(values = c("forestgreen", "turquoise", "hotpink")) +
  theme_minimal()
...
litter_ann_perplot_m2 %>%
  filter (species != "Leaf_All") %>%
  mutate(species = recode(species, "Leaf_ACRU" = "ACRU", "Leaf_FAGR" = "FAGR", "Leaf_LITU" = "LITU", "Leaf_Other" = "Other Spp"))
...
```{r}
# plot horizontal focal species by plot over time

# poster questions:
## can we see early senescence in the SW plot over time?
  ## the SW treatment is 
```
