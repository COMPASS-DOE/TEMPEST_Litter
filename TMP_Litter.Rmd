...
litter_annual_perplot <- litter_annual_fluxes %>%
  group_by(Plot, Year) %>%
  summarise(All_Leaf_perplot = sum(All_Leaf_peryear, na.rm = TRUE),
            .groups = "drop")
...
```{r}
# Join data sets
litter_precip_year <- left_join(litter_annual_perplot, climdat, by = "Year")
