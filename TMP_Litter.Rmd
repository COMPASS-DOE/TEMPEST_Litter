...
# Just as an EXAMPLE Generate for the entire year NOT IDEAL: this hard-codes dates which we don't want to do
...
litter_C_A_allyears %>%
  complete(Plot, Trap, Date_collected = days_wanted) %>%
  mutate(All_Leaf = na.approx(All_Leaf, na.rm = FALSE)) %>% 
  # add a column for year;for each year, compute cumulative flux
  mutate(Year_Collected = year(Date_collected)) %>% 
...
ggplot(litter_C_A_daily, aes(x = yday(Date_collected), y = All_Leaf_Cumulative, color = Year_Collected)) +
  geom_point()
